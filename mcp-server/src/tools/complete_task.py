...
async def complete_task_tool(tool_call: Dict[str, Any]) -> Dict[str, Any]:
    """
    MCP tool to mark an existing task as completed for a user.
    ...
    try:
        ...
        try:
            ...
            action = "completed" if completed else "marked as active"
            return {
                "success": True,
                "task": {
                    "id": str(task.id),
                ...
                "message": f"Successfully {action} task: {task.title}"
            }
        finally:
            db.close()
...
def get_complete_task_schema() -> Dict[str, Any]:
    """
    Returns the JSON schema for the complete_task tool.
